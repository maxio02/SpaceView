    ...
    switch (widget.sensorControl) {
      ...
      case SensorControl.AbsoluteOrientation:
        ...
        _orientationSubscription = motionSensors.absoluteOrientation.listen((AbsoluteOrientationEvent event) {
            Vector3 newOrientation = Vector3(event.yaw, event.pitch, event.roll);
            //low pass filter to avoid shaking 
          orientation = low_pass_filter(newOrientation, orientation, 0.08);
        });
        break;
      default:
    }
...
Vector3 low_pass_filter(Vector3 newValue, Vector3 previousValue, double alpha) {
  for (int i = 0; i < 3; i++) {
    double delta = newValue[i] - previousValue[i];
